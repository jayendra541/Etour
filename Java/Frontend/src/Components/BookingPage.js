...
const BookingPage = () => {
  ...
  const [date, setDate] = useState([]); // Initialize as 0
  ...
  const sub= sessionStorage.getItem("subid");

  console.log(sub,"sub");
  useEffect(() => {
    // Fetch posts based on subCatId
    fetch(`http://localhost:8080/api/byCategory/${sub}`)
      .then(res => {
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/api/dateMaster/category/${sub}`)
      .then(res => {
        if (!res.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      .then(result => {
        ...
        setDate(result); // Assuming result is an array of date objects
        if (result.length > 0) {
...
{/* {posts.map(post => {
  // Filter dates for the current post
  const filteredDates = date.filter(dat => dat.subCategoryMaster?.subCat_id === post.subCat_id);

  // Use a Set to ensure unique date entries (if needed)
  const uniqueDates = Array.from(new Set(filteredDates.map(dat => dat.departDate))).map(dateStr => {
    return filteredDates.find(dat => dat.departDate === dateStr);
  });

  // Render the filtered and unique dates
  return uniqueDates.length > 0 && (
    <div key={post.subCat_id}>
      {uniqueDates.map((dat, index) => (
        <Card.Text key={index}>
          <h3>Departure</h3>
          <hr/>
